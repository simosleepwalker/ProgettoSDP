...
public class NodeImpl extends NodeServiceGrpc.NodeServiceImplBase {
        ...
        synchronized (sync) {
            ...
            for (int i = 0; i < nodes.size(); i++)
                if (nodes.get(i).getId().equals(this.nextNodeId))
                    changeNext(nodes.get(mod((i-1),nodes.size())).getIp(),nodes.get(mod((i-1),nodes.size())).getPort(),nodes.get(i).getId(),nodes.get(i).getIp(),nodes.get(i).getPort());
        }
    }
        ...
        try {
            ...
            for (int i = 0; i < nodes.size(); i++) {
                if (nodes.get(i).getId().equals(this.id)){
                    changeNext(nodes.get(mod((i-1),nodes.size())).getIp(),nodes.get(mod((i-1),nodes.size())).getPort(),this.id,this.ip,this.port);
                    changeNext(this.ip,this.port,nodes.get(mod((i+1),nodes.size())).getId(),nodes.get(mod((i+1),nodes.size())).getIp(),nodes.get(mod((i+1),nodes.size())).getPort());
                }
            }
            if (nodes.size() == 1)
    ...
    //region Client
    public static void sendToken (String ip, Integer port, p2p.nodes.Node.Token token) throws InterruptedException {
        try {
